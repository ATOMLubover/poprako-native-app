...
/// Save (upsert) page units in a single transaction.
/// For each unit: attempt UPDATE; if no row updated, INSERT and update project stats.
...
/// Save page units into an existing transaction `trx`.
/// Caller must manage transaction commit.
pub async fn save_page_units_in_trx(
    ...
    let mut page_project_cache: std::collections::HashMap<String, String> = std::collections::HashMap::new();
    let mut stats_by_project: std::collections::HashMap<String, (i64, i64, i64, i64)> = std::collections::HashMap::new();
