import type { TeamProfile } from "../models/team";
    ...
    <div className={`team-card ${defaultProfile.isHidden ? "is-hidden" : ""}`}>
      ...
      <div className="card-header">
        <div className="avatar-wrapper">
          <div className="team-avatar-char">{defaultProfile.teamId ? defaultProfile.teamId.charAt(0) : "?"}</div>
          <div className="status-badge">
            <div className={`status-dot ${defaultProfile.isHidden ? "" : "active"}`}></div>
          </div>
        </div>

        <div className="team-info">
          <h3 className="team-name">{defaultProfile.teamId}</h3>
          <p className="team-signature" title={defaultProfile.signature}>{defaultProfile.signature}</p>
        </div>
      </div>
